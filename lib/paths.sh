...
GPS_MESH_UPGRADE_SERVICE="geoproxy-mesh-upgrade"
...
gps_apply_paths() {
	if [[ -n ${GPS_TEST_PREFIX:-} ]]; then
		...
		GPS_MESH_UPGRADE_UNIT_PATH="${base}/etc/systemd/system/${GPS_MESH_UPGRADE_SERVICE}.service"
		...
		GPS_MESH_UPGRADE_UNIT_PATH="/etc/systemd/system/${GPS_MESH_UPGRADE_SERVICE}.service"
	...
	GPS_MESH_CLUSTER_VERSION="${GPS_MESH_DIR}/cluster-version.json"
	GPS_MESH_UPGRADE_PENDING="${GPS_MESH_DIR}/upgrade-pending"
