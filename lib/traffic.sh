...
gps_cmd_traffic_check() {
	...
	# 月流量重置或用量低于停服线：清除熔断并自动启动
	if [[ ${TRAFFIC_TRIPPED:-0} == 1 ]]; then
		gps_traffic_resume_cleared auto
		return 0
	fi
...
gps_traffic_resume_cleared() {
	...
	gps_svc start
	if [[ $mode == auto ]]; then
		msg "$(_green "流量自动恢复") ${TRAFFIC_LAST_PCT}% < ${TRAFFIC_STOP_PCT}%，${GPS_SERVICE} 已启动"
		{
			echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) TRAFFIC_AUTO_RESUME pct=${TRAFFIC_LAST_PCT} stop=${TRAFFIC_STOP_PCT}"
		} >>"${GPS_LOG_DIR}/traffic.log" 2>/dev/null || true
	else
...
# 熔断中且实时用量低于停服线时清除标记（供 start/restart/upgrade 前调用）
gps_traffic_try_auto_resume() {
	gps_traffic_defaults
	[[ ${TRAFFIC_TRIPPED:-0} == 1 ]] || return 0
	[[ -n $KIWI_VEID && -n $KIWI_API_KEY ]] || return 1
	...
	gps_traffic_apply_parsed "$line"
	save_state
	[[ ${KIWI_ERR:-1} == 0 ]] || return 1
	[[ ${TRAFFIC_LIMIT_BYTES:-0} -gt 0 ]] || return 1

	local pct_int
	pct_int=$(awk -v p="$TRAFFIC_LAST_PCT" 'BEGIN{printf "%d", p+0}')
	((pct_int >= TRAFFIC_STOP_PCT)) && return 1
...
gps_assert_not_tripped() {
	...
	gps_traffic_defaults
	[[ ${TRAFFIC_TRIPPED:-0} == 1 ]] || return 0
	if gps_traffic_try_auto_resume; then
		return 0
	fi
	err "流量熔断中 (TRAFFIC_TRIPPED=1)。用量仍 ≥ ${TRAFFIC_STOP_PCT}% 或 API 不可用；请: $GPS_NAME traffic resume"
}
