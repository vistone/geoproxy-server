...
- 「本机 overlay …流量在 10.66.0.0/16 内路由」为 v0.2.68 前旧表述，改为
  「经 wg-ep 与其它 peer 的 overlay /32 互联；仅节点管理面，不承载代理流量」。
- UDP 探测为盲发（sendto 成功≠可达），原「OK（可达）」判定失真——实测会出现
  「探测全 OK + 握手 8 失败」的矛盾输出。改为「已发包（盲发无回执，不能证明可达；以 WG 握手统计为准）」。
- 判定分支：握手成功才给绿色「数据面可能流通」（并注明代理不经 WG）；仅有探测包发出/端口有动静
  一律降为黄色「WG 握手待确认」；探测发包失败提示改为「域名解析或本机出网异常」，并说明
  云 SG 拦截 UDP 的真实表现是「发包成功但握手失败」。
...
- **POST /v1/hook/github**（mesh 控制面 `:19527` TLS）：校验 `X-Hub-Signature-256`（`GPS_GITHUB_WEBHOOK_SECRET`），Release `published` 或 push tag `v*` 触发 `upgrade self --ver <tag>`。
